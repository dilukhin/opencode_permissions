...
5. **ssh_relay#47 — архитектурный результат достаточен для текущей локальной интеграции.** Обновлённый #49 готов к рассмотрению; его слияние не блокирует #34. Реализация RemoteProcessSpec/helper отложена по четырём условиям раздела 1. При их выполнении в отдельной задаче определить exact payload/host identity/route/helper capability, stdin/sudo framing и outcomes, затем transport fixtures. Upload + shell exec не объявлять structured remote argv.
